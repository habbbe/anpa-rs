...
fn bench_hubb() {
    ...
    let parse_action = right!(skip_inline!("Com:"), lift!(action, parse_name, parse_cmd));
    let parse_info = right!(skip_inline!("Info:"), lift!(info, parse_name, parse_cmd));
    let parse_separator = skip_inline!("Separator").map(|_| Item::Separator);
    let parse_space = skip_inline!("Space").map(|_| Item::Space);
    ...
    let ignore = or_diff!(empty(), skip_inline!('#'));
